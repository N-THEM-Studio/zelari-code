...
- **Desktop: tentacles are picked from the chat** — a new **Tentacles** pill in the composer holds delegation (Automatic / Prefer / Maximum / Lead only) and a role grid (Explorer, Builder, Checker, Planner) with model and thinking effort per role, plus a one-click reset. Same prefs as before, so existing picks carry over; Settings → Agents drops the Delegation and Sub-agent models cards and points to the pill.
- **Kraken tentacle loop capped at 2× its soft budget** — a tentacle used to be allowed `max(soft×3, soft+60)` turns (84 for a deep general with a nominal budget of 20 tool calls); the 3 tentacles past 2× in the last week were 82% of all tentacle prompt tokens. The harness now stops at 2× with a forced closing report (`ZELARI_KRAKEN_TENTACLE_LOOP_FACTOR`, 1–5).
- **Desktop: Council left the mode picker** — the Run mode pill, the default-mode setting, the overlay and `Ctrl+Shift+D` offer Kraken and Zelari only; saved council chats and defaults reopen as Kraken. The CLI `--mode council` is unchanged.
...
- **Kraken writers ignored uncommitted work** — a `general` tentacle's worktree started from HEAD, so a file the lead had just written (uncommitted) was invisible to it: writers edited the old committed version, looped ("your file lives outside my worktree"), and the squash merge back collided with the same uncommitted file and was rolled back. A dirty parent is now snapshotted (temporary index, parent untouched) and the tentacle's own edits are applied back with an atomic `git apply`, uncommitted. `ZELARI_KRAKEN_WORKTREE_SEED=head` restores the old start point.
- **Auto-verify on work that never landed** — when a general's merge-back fails, the `task` tool no longer spends a verify and a rework on the kept branch; it keeps the debt open and tells the lead where the work is and how to recover it.
- **`task` timeout threw away finished work** — the 45-minute registry timeout discards the whole result; the auto-verify chain now only gets the time left (minus a 60 s reserve), so a general that finished keeps its report even when its verification cannot complete.
- **Desktop: sidecar log silently disabled since 2.46.2** — the sidecar prewarm spawned the CLI before any AppHandle was known, so `zelari-sidecar.log` and the `harness-sidecar-log` events stayed off for the sidecar's whole lifetime. The prewarm now registers the handle first, and the stderr drain resolves it lazily.
...
Feature slice **cache-hit-rate** (plan tasks t127–t132): the request prefix is now stable by construction — the volatile segment (workspace / plan / RAG / durable state) travels as an ephemeral trailing message AFTER the history instead of as a second system message ahead of it, so a context change busts only the request tail while tools + identity stay cached. The trailing message lives in the HTTP body only (nothing new is persisted on the spine), and every number below comes from the provider, never from our own estimator. Gate: typecheck exit 0, 61 tests green across the eight touched suites, full suite 5827 passing (plus one load-induced 5s timeout in `tools/eval/runAnchors.test.ts`, green in isolation — unrelated to this slice).
...
- **`convergedClean` — degraded convergence is visible everywhere** — `KrakenExecutionSummary` and `ScriptRunResult` gain `convergedClean` (`converged && unresolvedFindings.length === 0`); the radio detail, the workbench event log and the graph digest now report `converged — degraded (N unresolved)` for a settled run with findings left behind (the `graph_converged` token stays the *prefix*, so existing desktop parsers keep matching). Presentation only: exit codes are unchanged (a degraded graph still exits 0 — the work exists and is merged, and failing the whole graph on one prompt drift would be the wrong trade) and merge semantics still ride `converged`. `SliceTrace.convergedClean?` is additive on mission state: absent means unknown (legacy), never coerced to `true`/`false`, never used to gate slice status.
- **One-shot verify re-ask on `unknown` (`ZELARI_KRAKEN_VERIFY_REASK`, default ON)** — a reviewer reply with an unparseable verdict trailer used to cost a full second review for one mangled line. The executor now sends a single non-streaming, no-tools completion carrying the reviewer's own prior output (tail-capped at 8000 chars, 12s budget) and asks only for the verdict the report already supports; the reply is parsed with the same `parseVerifyVerdict` grammar — no second grammar to drift — at most **once per node**. Fail-open throughout (disabled, no key, HTTP error, still unparseable → the existing unknown-flow), the node's raw `result` is never rewritten (audit sees what the reviewer actually emitted), and `unknown ≠ pass` is untouched.
- **Tentacle sidecars + the explore coverage metric** — every tentacle persists its full conclusion to `.zelari/radio/tentacles/<sessionId>/<nodeId>.md` (header: agent, thoroughness, model, duration, worktree; 16 KB cap; fail-open, never a dependency of the run). The reason is measurement honesty: `kraken radio` truncates a tentacle's `detail` (120 chars), and that truncation is biased *against* `medium`/`deep` explores — a terse `quick` conclusion survives it better — so a flip decided from the radio would be measuring its own artifact. Each session's sidecars now also yield a `coverage.json` metric (mentioned ∩ touched / touched).
- **Explore flip gate (`npm run explore:gate`)** — `tools/eval/runExploreFlipGate.ts` reads those reports and decides **with data, not intuition**: status-as-exit-code (0 keep / 1 revert / 2 insufficient-data), `--json`, `--dir`, `--min-sessions`, `--max-drop-pp`; zero deps, zero writes — it measures. Per phase it takes the **median** (one outlier must not move a default that changes every future run) and reverts only on a **strictly** greater-than-threshold drop, so exactly 10.0pp keeps. Below **≥5 valid sessions per phase** the answer is `insufficient-data`: an empty directory can never read as "flip safe". Sessions written before this change declare no `thoroughness` in their sidecar header, so they are **excluded**, never guessed into a phase; the header now records the tentacle's *actual* thoroughness (the `task` arg, threaded from the tool) and `coverage.json` carries `exploreModes` plus `exploreModesUnknown` — a missing `exploreModes` stays distinguishable from "this session had no explore at all".

### Changed

- **`maxNodes` is documented as the API parameter it is** — the JSDoc on `PlanTaskGraphOptions.maxNodes` (CLI planner) names the effective default `DEFAULT_MAX_NODES` (declared in `packages/core/src/kraken/graph.ts`) and states plainly that `ZELARI_KRAKEN_MAX_NODES` has no reader anywhere in `src/`; the phantom reference in `graph.ts` is gone, and the `{@link}` that pointed at a symbol five lines below became plain text. No behaviour change.
- **GUIDA** — `ZELARI_KRAKEN_VERIFY_REASK` joins the env table (default on, `0` disables) and a new paragraph explains reading the tentacle sidecars and the flip gate, including the ≥5-sessions and 10pp thresholds.
