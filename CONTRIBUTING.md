...
3. Cut the tag with `npm run tag-release -- vX.Y.Z --scope=<ack>`. It refuses to tag unless the working tree is clean, you are on `main` aligned with `origin/main`, the tag does not already exist, root `package.json` matches, `verify:versions` passes on the exact commit, **and** you declared a scope ack: `--scope=plan:<phase>` (`S0|S1|S2|S3|S4|S5|M1|M2` — the content is authorized by that phase of `ZELARI-2.37-NEXT.md` §4) or `--scope=exception:<reason>` (a written §5 surface exception, e.g. a justified Desktop fix). The declaration travels in the annotated tag message and is audited against the plan scorecard (§10). Example: `npm run tag-release -- v2.50.0 --scope=plan:M1`. Note the `--` separator: `npm` owns a `--scope` config flag of its own. Do **not** hand-run `git tag`/`git push` (that is how the v2.43.0 tag landed on a dirty tree).
