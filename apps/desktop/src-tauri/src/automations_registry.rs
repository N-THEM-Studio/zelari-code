...
/// Headless login-state probe for a channel. Exit 4 (relogin_required) is a VALID
/// answer, not a failure: the report is returned either way. If the CLI emits a
/// JSON report we pass it through verbatim; otherwise we synthesize one from the
/// exit code + the human line (health currently prints plain text).
#[tauri::command]
