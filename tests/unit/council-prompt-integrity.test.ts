...
/** Repo root — stable when vitest runs from root or `npm test --workspace=@zelari/core`. */
const REPO_ROOT = join(fileURLToPath(new URL("../..", import.meta.url)));
const ROLES_PATH = join(REPO_ROOT, "packages/core/src/agents/roles.ts");
const DIRECTIVES_PATH = join(
  REPO_ROOT,
  "packages/core/src/agents/councilDirectives.ts",
);
