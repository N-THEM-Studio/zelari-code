...
function makeStream(
  deltasPerCall: ReadonlyArray<Record<string, unknown>>,
): ProviderStreamFn {
...
describe("council chairman (Lucifero synthesis) — v0.6.0", () => {
  it('Lucifero emits agent_start with memberId="lucifer" when councilSize=6', async () => {
    ...
    const events = await collect(
      dispatchCouncil("summarize this", {
        apiKey: TEST_API_KEY,
        model: "grok-build",
        councilSize: 6,
    ...
    const luciferStart = events.find(
      (e) =>
        e.type === "agent_start" &&
        (e as { memberId?: string }).memberId === "lucifer",
    );
    expect(luciferStart).toBeDefined();
    expect((luciferStart as { memberName?: string }).memberName).toBe(
      "Lucifero",
    );
  });
  ...
  it("Lucifero emits member_cost with non-zero durationMs", async () => {
    ...
    const luciferCost = events.find(
      (e) =>
        e.type === "member_cost" &&
        (e as { cost: { memberId: string } }).cost.memberId === "lucifer",
    );
    expect(luciferCost).toBeDefined();
    const cost = (
      luciferCost as { cost: { durationMs: number; errored: boolean } }
    ).cost;
  ...
  it("Lucifero + 4 specialists + Minosse = 6 agent_start with memberName (Bug C fix: Minosse always runs)", async () => {
    ...
    const memberStarts = events.filter(
      (e) =>
        e.type === "agent_start" &&
        (e as { memberName?: string }).memberName !== undefined,
    );
    ...
    const last = memberStarts[memberStarts.length - 1] as {
      memberId?: string;
      memberName?: string;
    };
    ...
    const minosse = memberStarts.find(
      (e) => (e as { memberId?: string }).memberId === "minos",
    );
    expect(minosse).toBeDefined();
    expect((minosse as { memberName?: string } | undefined)?.memberName).toBe(
      "Minosse",
    );
  });
  ...
  it("Lucifero error does NOT abort the council run (robustness)", async () => {
    ...
    const stream: ProviderStreamFn = async function* (params) {
      const idx = callCount++;
      const isChairman = params.messages.some(
        (m) =>
          m.role === "system" &&
          typeof m.content === "string" &&
          m.content.includes("Lucifero"),
      );
      if (isChairman) {
    ...
    const finalEnd = events.find(
      (e) =>
        e.type === "agent_end" &&
        (e as { memberId?: string }).memberId === undefined,
    );
    ...
    expect((luciferCost as { cost: { errored: boolean } }).cost.errored).toBe(
      true,
    );
  });
  ...
  it("Regression HIGH-1: chairman catch() must NOT overwrite fullText (fallback message must render)", async () => {
    ...
    for await (const _ of runCouncilPure(
      "hello",
      {
        apiKey: TEST_API_KEY,
        model: "grok-build",
  ...
  it("Regression HIGH-4: specialist marks errored=true when AgentHarness emits an error event", async () => {
    ...
    for await (const _ of runCouncilPure(
      ...
      {
        onMemberCost: (cost) =>
          costs.push({ memberId: cost.memberId, errored: cost.errored }),
      },
    )) {
      // drain
    }
