...
function makeFetchMock(
  ...
  const fn = (async (url: string | URL | Request) => {
    ...
    const response =
      byPredicate ?? responses[Math.min(callIndex, responses.length - 1)];
...
describe("requestDeviceCode (RFC 8628)", () => {
  ...
  it("throws GrokOAuthError when device_code is missing", async () => {
    ...
    await expect(
      requestDeviceCode({ clientId: "c", fetchImpl: fetchMock }),
  ...
  it("throws GrokOAuthError on HTTP error", async () => {
    ...
    await expect(
      requestDeviceCode({ clientId: "bad", fetchImpl: fetchMock }),
...
describe("pollForDeviceToken (RFC 8628 §3.4)", () => {
  ...
  it("throws GrokOAuthError (denied) when user denies authorization", async () => {
    const fetchMock = makeFetchMock([
      { body: { error: "access_denied" }, status: 400 },
    ]);
    await expect(
      pollForDeviceToken({
        clientId: "c",
  ...
  it("throws GrokOAuthError (expired) when device code expires", async () => {
    const fetchMock = makeFetchMock([
      { body: { error: "expired_token" }, status: 400 },
    ]);
    await expect(
      pollForDeviceToken({
        clientId: "c",
...
describe("runGrokOAuthFlow (device flow, fully mocked)", () => {
  it("completes device flow end-to-end and calls onUserCode with the code", async () => {
    ...
    const fetchMock = makeFetchMock([
      // device-code request
      {
        ...
        body: {
          ...
          verification_uri: "https://auth.x.ai/device",
          verification_uri_complete:
            "https://auth.x.ai/device?user_code=ZZZZ-9999",
          expires_in: 900,
          interval: 5,
        },
    ...
    // Stub the browser launch — without this the test pops a REAL browser
    // tab at auth.x.ai with the fake user_code on every `npm test` run.
    ...
    // verification_uri_complete (pre-filled code) is preferred for the browser.
    expect(openedUrl).toBe("https://auth.x.ai/device?user_code=ZZZZ-9999");
  });
});
...
describe("refreshGrokToken", () => {
  ...
  it("throws GrokOAuthError on missing refreshToken", async () => {
    await expect(
      refreshGrokToken({ clientId: "c", refreshToken: "" }),
...
describe("default constants", () => {
  it("uses the xAI public OAuth client", () => {
    expect(DEFAULT_GROK_OAUTH_CLIENT_ID).toBe(
      "b1a00492-073a-47ea-816f-4c329264a828",
    );
  });
  it("device-code endpoint points at auth.x.ai", () => {
    expect(DEFAULT_DEVICE_CODE_ENDPOINT).toBe(
