...
import { probeSessionCookiesOnDisk, type DiskCookieProbe } from './cookieDisk.js';
...
export interface CheckLoginDeps {
  ...
  cwd?: string;
  navTimeoutMs?: number;
  /** Injected cookie-DB probe (tests). Production reads the profile on disk. */
  diskProbe?: (profile: string, wanted: readonly string[]) => Promise<DiskCookieProbe>;
}
...
export async function checkLogin(channel: string, deps: CheckLoginDeps = {}): Promise<HealthReport> {
  ...
  // Skip the default disk probe when a test injects `opener` (otherwise we'd
  // read the real user profile). An explicit `diskProbe` always runs first.
  const probe = deps.diskProbe ?? (deps.opener ? undefined : probeSessionCookiesOnDisk);
  if (probe && wanted.length > 0) {
    const disk = await probe(profileDir(channel), wanted);
    if (disk === 'present') {
      return { channel, loggedIn: true, matched: `cookies:disk:${wanted.join('+')}`, checkedAt: nowIso() };
    }
    if (disk === 'locked') {
      // Chromium already has the profile — not logged-out. Don't fight it.
      return { channel, loggedIn: true, matched: 'profile-in-use', checkedAt: nowIso() };
    }
    if (disk === 'missing') {
      return { channel, loggedIn: false, matched: 'cookies:missing-on-disk', checkedAt: nowIso() };
    }
  }
