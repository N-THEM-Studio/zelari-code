    ...
    case 'evolve': {
      ...
      return {
        ...
        message: [
          ...
          ...(stats.runs > 0 ? [`  verdicts: ${verdicts || '—'}`, `  classes: ${classes || '—'}`] : []),
          ...(stats.runs > 0 && stats.lastAt ? [`  last run: ${stats.lastAt}`] : []),
          'Proposals/promotions stay in tools/eval evolve:propose/evolve:decide — nothing self-promotes (P1).',
        ].join('\n'),
      };
    }
    case 'exit':
