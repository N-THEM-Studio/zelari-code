...
export interface SlashCommandResult {
  ...
  graphPrompt?: string;
  /** For `kraken_fanout`: the rest of the slash args (positional + flags). */
  fanoutArgs?: string;
  /** For `kraken_workbench`: the rest of the slash args (currently unused). */
  ...
  modeTarget?: 'kraken' | 'council' | 'zelari';
  ...
  /** For `state_commit` / `state_show` / `state_restore`: optional label or commit id. */
  stateArg?: string;
  /** For `state_restore`: skip linked git tree restore when true. */
  stateNoTree?: boolean;
  /** For `/memory`: subcommand and remaining arguments. */
  memorySubcommand?: string;
  memoryArgs?: string[];
  /** For `trust_*`: the folder path to trust/remove (default: cwd). */
  trustPath?: string;
...
export function handleSlashCommand(
  ...
  /inbox — what is waiting on you across local sessions (unanswered ask_user, needs input, tentacle finished; ZELARI_INBOX=0 disables)
  ...
  /help — show this help\n  /exit — exit the CLI\n\n${formatSkillList(availableSkills)}`,
    ...
    case 'evolve': {
      // Evolution surface (ADR-0036). Read-only: the only I/O in this parser,
      // sync + fail-open on purpose — telemetry, never a gate. Sub-commands:
      //   /evolve            — ledger status + deterministic fitness (t42)
      //   /evolve proposals  — folded view of the tools/eval proposal store (t43)
      const sub = (args[0] ?? 'status').toLowerCase();
      const mode = evolutionMode();
      if (sub === 'proposals') {
        const records = readProposalStore(process.cwd());
        const sum = proposalSummary(records);
        const lines = records
          .slice(-20)
          .map(
            (p) =>
              `  ${p.id} [${p.status}] ${p.operator ?? '?'} · ${p.surface ?? '?'} · evidence ${
                p.evidenceCount ?? 0
        ...
        return {
          ...
          kind: 'evolve',
          message: [
            `Evolution proposals — .zelari/evolution/proposals.jsonl — ${sum.total} total (${Object.entries(
              sum.byStatus,
            )
              .map(([k, v]) => `${k} ${v}`)
              .join(', ') || '—'})`,
            ...(lines.length > 0 ? lines : ['  (no proposals yet — run npm run evolve:propose)']),
            ...(records.length > 20 ? [`  … +${records.length - 20} older (see the store)`] : []),
            'Decisions stay in npm run evolve:decide — the TUI never promotes anything (P1).',
          ].join('\n'),
        };
      }
      const entries = readLedger(process.cwd());
      const stats = ledgerStats(entries);
      const verdicts = Object.entries(stats.byVerdict).map(([k, v]) => `${k} ${v}`).join(', ');
      const classes = Object.entries(stats.byClass).map(([k, v]) => `${k} ${v}`).join(', ');
      const fitness = Object.entries(stats.byClassFitness)
        .map(
          ([cls, f]) =>
            `  ${cls}: pass ${f.passRate.toFixed(2)} · weighted ${f.weightedPassRate.toFixed(2)} · rollback ${f.rollbackRate.toFixed(2)}${f.avgCostUsd !== undefined ? ` · $${f.avgCostUsd.toFixed(3)}` : ''}`,
        )
        .join('\n');
      return {
        handled: true,
        kind: 'evolve',
        message: [
          `Evolution Engine v0 (ADR-0036) — mode: ${mode === 'shadow' ? 'shadow (ledger ON)' : 'off (set ZELARI_EVOLUTION=shadow to enable)'}`,
          `Ledger: .zelari/evolution/ledger.jsonl — ${stats.runs} run(s)`,
          ...(stats.runs > 0
            ? [
                `  verdicts: ${verdicts || '—'}`,
                `  classes: ${classes || '—'}`,
          ...
          'Fitness is deterministic (tier-weighted, ADR-0023) — no LLM judges (P1).',
          'See also: /evolve proposals · npm run evolve:propose/decide — nothing self-promotes.',
        ].join('\n'),
      };
    }
    case 'exit':
    ...
    case 'resume-mission': {
      // experimental/cursor-learn 2.3 — TUI twin of headless --resume-mission
      // and of the Desktop "Riprendi" pill (same resumeZelariMission seam).
      if (args[0] === 'status') {
        return {
          handled: true,
          kind: 'resume_mission_status',
          message: '[zelari] reading .zelari/mission-state.json …',
        };
      }
      return {
        handled: true,
        kind: 'resume_mission',
        message:
          'Usage: /resume-mission [status] — resume the persisted mission, or show its status',
      };
    }
    ...
    case 'skills': {
      // `/skills` — always open the interactive skill picker (list + select).
      return {
        handled: true,
        kind: 'skill_picker',
    ...
    case 'skill': {
      ...
      if (!skillId) {
        // No id → interactive picker (fallback: text list when no TUI picker).
        return {
          handled: true,
          kind: 'skill_picker',
          message: formatSkillList(availableSkills),
    ...
    case 'kraken': {
      ...
      // `/kraken fanout <csv> --col ... --out ... --instruction ...`
      if (args[0] === 'fanout') {
        return {
          handled: true,
          kind: 'kraken_fanout',
      ...
      // `/kraken workbench` → print the live workbench to the transcript
      if (args[0] === 'workbench') {
        return {
          handled: true,
    ...
    case 'mode': {
      // `/mode`                → cycle kraken → council → zelari
      // `/mode <kraken|council|zelari>` → set directly (agent = alias for kraken)
    ...
    case 'statusline': {
      // `/statusline …` (t114) — configure the status line. The handler does
      // the read-modify-write on the prefs file and returns the text to show;
      // an unknown sub-command still prints the current state + usage.
      return { handled: true, kind: 'statusline', message: handleStatusLine(args) };
    }

    case 'report': {
      // `/report [sessionId]` (t115) — aggregate view of the active session
      // spine. Fail-soft: a missing/corrupt spine renders a clear message.
      const target = args.find((a) => !a.startsWith('-'));
      return {
        handled: true,
        kind: 'report',
        message: buildSessionReport(target ? { sessionId: target } : {}).markdown,
      };
    }

    case 'inbox': {
      // `/inbox` — derive-only scan of the LOCAL spine sessions for ask_user
      // questions that were asked and never settled. Reads, never writes;
      // fail-soft (a broken session is skipped, never thrown) and gated by
      // ZELARI_INBOX=0, which reports "disabled" instead of printing a list.
      return { handled: true, kind: 'inbox', message: renderInbox() };
    }
    ...
    case 'state': {
      // `/state` | `/state status` | `/state commit [label]` | `/state show [id]`
      // `/state restore [id] [--no-tree]`
      const sub = (args[0] ?? 'status').toLowerCase();
      if (sub === 'status' || sub === '') {
        return { handled: true, kind: 'state_status' };
      }
      if (sub === 'commit') {
      ...
      if (sub === 'restore') {
        ...
        const noTree = rest.some((a) => a === '--no-tree' || a === '-n');
        const id = rest.find((a) => !a.startsWith('-'))?.trim();
        return {
          handled: true,
          kind: 'state_restore',
          ...(id ? { stateArg: id } : {}),
          ...(noTree ? { stateNoTree: true } : {}),
        };
      }
      return {
        handled: true,
        kind: 'state_usage',
        message:
          'Usage: /state status | /state commit [label] | /state show [id] | /state restore [id] [--no-tree]\n' +
    ...
    case 'cache': {
      ...
      if (sub === 'stats' || sub === '') {
        return { handled: true, kind: 'cache_stats' };
      }
      return {
        handled: true,
        kind: 'cache_stats',
        message: 'Usage: /cache stats — session prompt-cache hit rate and premium vs cached tokens.',
      };
    }
