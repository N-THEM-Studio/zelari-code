...
**Cost on Windows:** every worktree is a full checkout of the tracked tree (measured ~2s / 1740 files on this repo). That is per writer and amortized over the whole tentacle, but on very short tasks in very large repos the copy can outweigh the benefit — that is what `ZELARI_KRAKEN_WORKTREE=0` is for.

Teardown is hardened for Windows (2.39): `git worktree remove --force` and the filesystem sweep are retried (3 attempts, short backoff) because a file still held by git/an antivirus/an editor fails with `EPERM`/`EBUSY`; the recursive sweep only ever touches a `kraken-*` directory inside `<repo>/.zelari/worktrees/` (`.zelari/` is gitignored, so user files can never be reached); and the branch is deleted only once the directory is actually gone — if the directory survives, the branch is kept instead of orphaning the work. Long paths switch on demand past 240 chars (`\\?\` for the fs, `-c core.longpaths=true` for git).
