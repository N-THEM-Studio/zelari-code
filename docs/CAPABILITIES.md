...
| **System reminder** (per-turn reminder block) | planned | `packages/core/src/core/modules/system-reminder/systemReminder.ts` + `systemReminder.test.ts` — module only: `AgentHarness.ts` states the projection seam is the CLI budget pipeline (ADR-0032) and carries the `TODO(seam)` on `messagesForProvider()`. No runtime wiring yet |
