...
surface the same status contract the TUI now shows (Fase / Modo / Verifica /
Permessi). Two products with two different stories is an identity hole: the user
...
2. **Now — same contract chips.** The Desktop status strip derives the four
   chips (Fase, Modo, Verifica `prova: PASS|RIPARA|BLOCCATO`, Permessi) from
